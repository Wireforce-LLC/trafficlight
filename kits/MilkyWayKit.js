...
class MilkyWayKit {
  ...
  async resolveRouter(routerName, {req, res}) {
    ...
    if (!_.isObject(routerObject)) {
      $loggerKit.getLogger().debug(`Router '${routerName}' is not working correctly. Router content is invalid`)
      return null
    }
    ...
    if (_.isBoolean(if_)) {
      $loggerKit.getLogger().debug(`Router '${routerName}' used '() => ${if_}' as 'if' segment`)
      if_ = () => if_
    }

    if (_.isNull(else_) || _.isUndefined(else_)) {
      $loggerKit.getLogger().debug(`Router '${routerName}' used '{} as 'else' segment`)
      else_ = {}
    }

    if (!_.isObject(if_) && !_.isFunction(if_)) {
      $loggerKit.getLogger().debug(`Router ${routerName} is not working correctly. There is no 'if' field or it is invalid`)
      return null
    }

    if (!_.isObject(then_)) {
      $loggerKit.getLogger().debug(`Router ${routerName} is not working correctly. There is no 'then' field or it is invalid`)
      return null
    }

    if (!_.isObject(else_)) {
      $loggerKit.getLogger().debug(`Router ${routerName} is not working correctly. There is no 'else' field or it is invalid`)
      return null
    }

    if (!_.isArray(filters_)) {
      $loggerKit.getLogger().debug(`Router ${routerName} is not working correctly. Filters are specified incorrectly.`)
      return null
    }
    ...
    if (_.isArray(_.get(if_, 'tools', []))) {
      for (const tool of _.get(if_, 'tools', [])) {
        ...
        const args = Filter.maskObject(toolData[1], {
          ..._.mapKeys(_.get(req, 'query', {}), (_, i) => "$get." + camelCase(i)),
        })
    ...
    $loggerKit.getLogger().warn(`Router '${routerName}' has [${booleanCollections}]`)

    if (!_.includes(booleanCollections, false) && !_.includes(booleanCollections, undefined)) {
